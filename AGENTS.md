...
- **Технологический шлюз жёсткий.** Указан пакет проекта — совпадение
  обязательно; координация по IFC его не заменяет (п.8).
- **AI — менеджер, а не исполнитель.** Следит за сроками и хендоффами,
  чертежей не делает (п.12а).
