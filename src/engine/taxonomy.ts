...
/*
 * Климат опыта, а не климат проекта. Проекты бюро пока лежат в тридцати
 * странах, но специалист мировой, и архитектор, строивший в тропиках, должен
 * иметь возможность об этом сказать, а не выбирать ближайшее неверное.
 */
...
/**
 * Разделы, под которыми закон требует подписи ответственного проектанта.
 *
 * Подпись ставится не одна на весь комплект, а по каждому разделу. Так в
 * Черногории: проектант до начала работы назначает ведущего проектанта и
 * ответственного проектанта по каждому виду проекта и уведомляет министерство,
 * а каждый ответственный подписывает свою часть. Так же в Германии, где расчёт
 * конструкций подписывает отдельный человек из списка конструкторов, и в
 * Италии, где проект железобетона подписывает инженер из реестра.
 *
 * Раньше движку хватало одного подписанта на команду. Это было неверно: пакет
 * с подписью архитектора и без подписи конструктора орган не примет.
 *
 * Ландшафт, визуализация, интерьер сюда не входят: это не виды проекта, под
 * которыми нужна подпись для разрешения. Согласования и геодезия проверяются
 * своим гейтом страны (LOCAL_ONLY_DISCIPLINES).
 */
...
export const LANGUAGES = [
  ...
  'sk', 'sl', 'es', 'sv', 'is', 'no',
  // Языки пула за пределами юрисдикций. Пул мировой, а язык команды
  // считается пересечением: человек, чей язык ни с кем не совпал, просто не
  // получит в пару соотечественника, но и не будет отрезан от анкеты.
  'ar', 'tr', 'uk', 'hy', 'ka', 'az', 'kk', 'uz', 'fa', 'he', 'hi', 'ur', 'bn', 'zh', 'ja', 'ko', 'id', 'ms', 'th', 'vi', 'sq', 'mk', 'bs', 'sw',
] as const
...
/**
 * Все смещения от UTC, которые где-то в мире действуют.
 *
 * Целые часы от −12 до +14 и девять дробных: +3:30 Иран, +4:30 Афганистан,
 * +5:30 Индия, +5:45 Непал, +6:30 Мьянма, +9:30 Центральная Австралия,
 * +10:30 остров Лорд-Хау, −3:30 Ньюфаундленд, −9:30 Маркизские острова.
 * Список закрытый, чтобы опечатка не превращалась в несуществующий пояс.
 */
export const UTC_OFFSETS: readonly number[] = [
  ...Array.from({ length: 27 }, (_, i) => i - 12),
  -9.5, -3.5, 3.5, 4.5, 5.5, 5.75, 6.5, 9.5, 10.5,
].sort((a, b) => a - b)

/** Как смещение пишется человеку: UTC+5:30, UTC−3:30, UTC+0. */
export function utcOffsetLabel(offset: number): string {
  const sign = offset < 0 ? '−' : '+'
