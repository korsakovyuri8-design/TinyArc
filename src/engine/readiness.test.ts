...
import { DISCIPLINES, JURISDICTIONS } from './taxonomy'
...
describe('готовность на большой базе', () => {
  ...
  /*
   * Готовность по стране — второй такой же перебор, и он оставался без памяти,
   * пока `gaps` её уже имел. Замерено на живом Postgres: страница пула на базе
   * в пять тысяч человек отдавалась 1,8 секунды, и 1,7 из них уходило сюда —
   * при том что подписей роли всего девятнадцать, а форм проекта сотни.
   */
  it('готовность по стране считается за отведённое время', () => {
    /*
     * Пул нарочно широкий: у каждого все дисциплины.
     *
     * Дорог здесь не найденный ответ, а ненайденный. Совпадение по дисциплине
     * проходит у всех, и решает специализация — то есть проверка идёт по
     * каждому из пяти тысяч до конца. Пул из узких профилей отвечает на первом
     * же человеке и не измеряет ничего: с ним разница между памятью и
     * перебором вчетверо, а не в шестнадцать раз.
     */
    const pool = Array.from({ length: 5000 }, (_, i) =>
      specialist({ id: `all-${i}`, disciplines: [...DISCIPLINES], portfolioRating: 9 }),
    )

    const started = performance.now()
    for (const jurisdiction of JURISDICTIONS) readiness(pool, jurisdiction)
    const spent = performance.now() - started

    // На этой машине — около двадцати миллисекунд на все страны, без памяти —
    // триста. Запас десятикратный: проверка ловит возврат к перебору на каждом
    // появлении роли, а не медленный день сборочной машины.
    expect(spent, `готовность заняла ${spent.toFixed(0)} мс`).toBeLessThan(200)
  })
