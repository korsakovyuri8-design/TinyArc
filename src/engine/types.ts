...
/**
 * Местная проектная фирма с правом подписи.
 *
 * Не участник команды: она не проектирует, а проверяет сделанное командой,
 * подписывает разделы своими лицензированными инженерами и подаёт пакет. В
 * Черногории без такой фирмы документацию не выпустить вовсе: выпускать её
 * может только проектант, у которого в штате есть лицензированный архитектор
 * или инженер по нужному виду проекта.
 *
 * Отдельным типом, а не специалистом с флагом, потому что это другое
 * отношение: фирма не проходит гейты, не набирает балл и не занимает слот.
 * Она либо закрывает подпись раздела в своей стране, либо нет.
 */
export type SigningPartner = {
  id: string
  name: string
  jurisdiction: Jurisdiction
  /** Разделы, которые фирма вправе подписать: по лицензиям её сотрудников. */
  disciplines: Discipline[]
}

/** Кто подписывает раздел: участник команды или партнёрская фирма. */
export type SignOff =
  | { discipline: Discipline; by: 'member'; specialistId: string }
  | { discipline: Discipline; by: 'partner'; partnerId: string }
...
export type Assembly = {
  ...
  /**
   * Кто подписывает каждый раздел, где нужна подпись. Пусто, если состав не
   * собрался.
   */
  signOff: SignOff[]
  /**
   * Разделы, под которыми подписать некому. Не пусто только при
   * `no_signatory`: по нему бюро видит, какую именно подпись искать.
   */
  unsigned: Discipline[]
}
