...
export type BriefParse = {
  ...
  notes: string
  /**
   * То же описание по-английски, для команды.
   *
   * Заказчик пишет на своём языке, команда собрана со всего мира и работает на
   * английском. Перевод делается один раз здесь, а не каждым специалистом
   * наугад в разделе, который идёт под его подпись.
   */
  summary: string
}
