...
describe('договор с моделью', () => {
  ...
  /*
   * Расход наружу считается на месте обращения и больше нигде.
   *
   * Выше по стеку токенов уже нет: их возвращает только ответ провайдера.
   * Пропущенная ветка отказа — это не потерянная строка в журнале, а расход,
   * которого по журналу не было: оборванный по потолку ответ оплачен целиком.
   */
  it('каждое обращение и каждый отказ попадают в журнал расхода', () => {
    expect(source, 'запись расхода пропала').toContain('recordModelCall')

    for (const outcome of ['ok', 'rate_limit', 'auth', 'network', 'provider', 'refusal', 'truncated', 'schema']) {
      expect(source, `исход «${outcome}» не записывается`).toContain(`note('${outcome}'`)
    }
  })

  it('каждый помощник называет себя при обращении', () => {
    for (const method of ASSISTANT_METHODS) {
      expect(source, `${method} уходит в модель без имени: расход не разложить`).toContain(
        `'${method}',`,
      )
    }
  })
  ...
  /*
   * Чужой текст и указания бюро не должны лежать в одном плоском промпте.
   *
   * Заметка специалиста, описание клиента, ветка спора и названия файлов
   * написаны людьми, и для модели строка внутри них ничем не отличается от
   * указания. Самый дорогой случай — черновик запроса смежнику: это
   * единственный путь, по которому текст одного специалиста доходит до
   * другого, а прямых каналов в продукте нет по замыслу.
   */
  it('весь чужой текст входит в промпт огороженным', () => {
    expect(source, 'ограждение чужого текста пропало').toContain('function untrusted(')
    ...
    for (const field of [
      ...
      const fenced = new RegExp(`untrusted\\([^)]*${field.replace('.', '\\.')}`)
      expect(source, `${field} уходит в модель без ограждения`).toMatch(fenced)
    }
  })
