...
describe('архив', () => {
  ...
  /*
   * Потолок проверяется настоящим переходом через него, и дешевле это не
   * делается: предел зашит в формат — шестнадцать бит на число записей, — и
   * доказать отказ можно только шестьюдесятью пятью тысячами записей.
   * Отсюда и свой срок: пять секунд по умолчанию эта проверка проходила
   * впритык и падала на занятой машине, объявляя поломкой загрузку сборщика.
   */
  it('слишком много записей — отказ, а не молча битый архив', { timeout: 60_000 }, async () => {
    async function* many() {
