...
function russian(source: string): string[] {
  ...
  /*
   * Текст разметки ищется четырьмя кусками, а не одним.
   *
   * Раньше был один — от `>` до `<` без фигурных скобок внутри. Любая вставка
   * разрывает такой кусок, а вставки в тексте обычны: `{' '}` на переносе
   * строки, подставленный адрес, число. Через эту щель на первый экран
   * заказчика — тот, где ему выдают ключ доступа, — уехали две русские фразы
   * и прожили там до прогона на пустом стенде.
   *
   * Поэтому фигурная скобка считается границей куска, а не запретом: текст
   * берётся и до вставки, и после неё, и между двумя. Само содержимое вставки
   * при этом ловят шаблоны строк выше.
   */
  const patterns = [
    /'((?:[^'\\\n]|\\')*)'/g,
    /"([^"\n]*)"/g,
