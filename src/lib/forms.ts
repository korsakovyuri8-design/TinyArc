...
import { UTC_OFFSETS } from '@/engine/taxonomy'
import { z } from 'zod'
...
export const applicationSchema = z.object({
  ...
  /*
   * Необязательно. Движок требует страну проекта только от согласователя и
   * геодезиста (LOCAL_ONLY_DISCIPLINES); архитектор, конструктор, инженер
   * систем подходят проекту из любой точки мира. Обязательное поле заставляло
   * человека без европейских объектов либо уйти, либо отметить страну, где он
   * не работал, то есть соврать в первом же ответе.
   */
  jurisdictions: z.array(z.enum(JURISDICTIONS)).default([]),
  signsIn: z.array(z.enum(JURISDICTIONS)),

  /**
   * Заявленные лицензии, по одной на каждую страну подписи.
   *
   * Номер не проверяется ни шаблоном, ни длиной. Сербия успела сменить
   * поколение шифров, у одного человека в реестре лежат номера обоих сразу, и
   * регулярное выражение по такому справочнику отвергало бы настоящие
   * документы. Проверка тут одна: номер есть или его нет.
   */
  licences: z
    .array(
      z.object({
        jurisdiction: z.string().refine(isCountry, 'Unknown country'),
        authority: trimmed.max(200),
        number: trimmed.max(120),
        registryUrl: trimmed.max(2000).default(''),
      }),
    )
    .default([]),

  /*
   * Контакты необязательны. Требовать телефон значило бы отсеивать тех, кто
   * даёт его после первого разговора, а не до,, то есть ровно тех взрослых
   * осторожных людей, ради которых всё и затевалось.
   */
  linkedinUrl: trimmed.max(2000).default(''),
  phone: trimmed.max(40).default(''),
  phoneWhatsapp: z.coerce.boolean().default(false),
  ...
  /*
   * Смещение дробное и только из настоящих: целые часы плюс те пояса, что
   * живут со сдвигом в полчаса и три четверти. Произвольная дробь вроде 3,7
   * означала бы опечатку, а не страну.
   */
  utcOffset: z.coerce.number().refine((v) => UTC_OFFSETS.includes(v), 'Choose your time zone from the list'),

  /** Страна проживания: любая страна мира, не юрисдикция движка. */
  residenceCountry: z.string().refine(isCountry, 'Choose the country you live in'),
