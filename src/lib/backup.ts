...
export const TABLES = [
  ...
  'PayoutRate',
  // Настройки бюро ни от чего не зависят. Восстановленная база без доли,
  // идущей команде, собирает состав без бюджетного гейта, и делает это
  // молча, потому что «не задано» и «не восстановлено» выглядят одинаково.
  'Setting',
  ...
  'Contractor',
  'ContractorTrade',
  // Фирмы с правом подписи ни от чего не зависят. Без них восстановленная база
  // отвечает «подписать некому» на каждый проект страны, и выглядит это не
  // как потеря данных, а как закрытая страна.
  'SigningPartner',
  'Specialist',
  // Ставки человека идут сразу за ним: без них его цена читается как
  // неназванная, и он молча выпадает из бюджетного расчёта.
  'SpecialistRate',
  'Project',
  'MatchRun',
  'Candidate',
...
export const SKIPPED: Record<string, string> = {
  ...
  ModelCall: 'журнал расхода наружу: ничем не владеет и ни на что не влияет, а восстановленный смешал бы траты до аварии с тратами после',
}
...
/**
 * Ключ страницы по таблицам, из самой схемы, а не «у всех есть `id`».
 *
 * Выгрузка идёт страницами, а страница без порядка на разных базах отдаёт
 * разное и теряет строки между запросами. Порядок брался по `id`, и это
 * держалось на том, что первичный ключ у всех таблиц назывался одинаково.
 * У настроек бюро он называется `key`, и выгрузка упала целиком: не потеряла
 * одну таблицу, а не собралась вовсе, то есть копии не стало в тот день,
 * когда в схеме появилась таблица с другим именем ключа.
 *
 * Составного ключа (`@@id`) здесь нет намеренно: страницу по нему надо
 * упорядочивать двумя полями, и молча взять первое значило бы вернуться к той
 * же догадке. Такая таблица обязана сначала объявить свой порядок.
 */
export function idFields(schema: string): Map<string, string> {
  ...
  for (const match of schema.matchAll(/\nmodel\s+(\w+)\s*\{([\s\S]*?)\n\}/g)) {
    ...
    for (const line of body!.split('\n')) {
      const field = line.trim().match(/^(\w+)\s+\S+\s.*\B@id\b/)
      if (field) result.set(model!, field[1]!)
    }
  }
