...
/**
 * Видимость стенда для поисковиков.
 *
 * Закрыта по умолчанию: первая выкладка идёт на канонический адрес продукта и
 * без этого попадает в индекс бюро без единого специалиста. Выпасть из выдачи
 * потом занимает недели.
 *
 * Проверяется умолчание, а не оба режима: открытый включается переменной
 * окружения, а конфигурация читает её на сборке — значит в одном прогоне
 * существует только одно её значение.
 */
describe('видимость стенда', () => {
  it('по умолчанию закрыт от индексации', async () => {
    expect(process.env.BUREAU_INDEXABLE ?? '').not.toBe('yes')

    const rules = await config.headers!()
    const everywhere = rules.find((r) => r.source === '/:path*')

    expect(everywhere, 'правило на все пути пропало').toBeDefined()
    expect(everywhere!.headers.map((h) => h.key)).toContain('X-Robots-Tag')
  })
})
