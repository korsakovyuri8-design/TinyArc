...
/**
 * Индексы, добавленные вместе с очередями, которых раньше не было.
 *
 * Та же логика, что и у трёх выше: запрос живёт на странице, которую бюро
 * открывает по многу раз в день, и перебирает таблицу, растущую всю жизнь
 * бюро. Принятые тикеты и обращения к моделям не удаляются никогда.
 */
describe('индексы под очереди, добавленные позже', () => {
  /*
   * Расхождение между приёмкой и начислением ищется по окну в тридцать дней.
   * Без пары «статус + время приёмки» база всё равно обходит все тикеты за всю
   * историю, чтобы отобрать свежие: окно без индекса не экономит ничего.
   */
  it('свежие приёмки ищутся по состоянию и времени приёмки', () => {
    expect(model('Ticket')).toContain('@@index([status, acceptedAt])')
  })

  /*
   * Расход за неделю считается сворачиванием в базе, и отбор по времени —
   * первое, что она делает.
   */
  it('расход на модели читается по времени обращения', () => {
    expect(model('ModelCall')).toContain('@@index([at])')
  })
})
